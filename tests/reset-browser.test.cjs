...
(async () => {
  ...
  try {
    ...
    const open = async () => {
      await page.locator('.btn-maintenance').click();
      assert.equal(await page.locator('.btn-backup,.btn-restore,.btn-cleanup,.btn-reset-card').count(), 4);
      const backupEvent = page.waitForEvent('download');
      await page.locator('.btn-backup').click();
      const backupDownload = await backupEvent;
      assert.match(backupDownload.suggestedFilename(), /^weekly-schedule-card-backup-\d{4}-\d{2}-\d{2}\.json$/);
      const backupStream = await backupDownload.createReadStream();
      let backupJson = ''; for await (const chunk of backupStream) backupJson += chunk;
      assert.equal(JSON.parse(backupJson).schema, 'weekly-schedule-card/backup');
      await page.locator('.wsc-modal-ok').click();
    ...
    assert.equal(await page.locator('.reset-confirm').isEnabled(), false, 'old phrase must no longer work');
    await page.locator('.reset-ack').uncheck();
    assert.equal(await page.locator('.reset-confirm').isEnabled(), false);
    await page.locator('.reset-ack').check();
    await page.locator('.reset-phrase').fill('reset');
    ...
    assert.equal(await page.locator('.btn-maintenance').count(), 0);
