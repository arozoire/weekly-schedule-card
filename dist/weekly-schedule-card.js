(function () {
  ...
  class WeeklyScheduleBase extends HTMLElement {
    constructor() {
      super();
    ...
    async _syncConditionAutomation(scheduleEntityId, conditions, condCombinator, condInterval=15, ps=null) {
      ...
      const automationConfig = {
        ...
        trigger: triggers,
        condition: [
          { condition: 'state', entity_id: scheduleEntityId, state: 'on' },
    ...
    async _syncNotifyAutomation(scheduleEntityId, ps) {
      ...
      // Template triggers via state_attr (robust: returns None for missing OR null attr),
      // tagged with id so the action can tell slot-start from slot-end.
      const triggers = [];
      const choose = [];
      if (trig === 'start' || trig === 'both') {
        triggers.push({ platform: 'template', value_template: `{{ state_attr('${scheduleEntityId}', 'current_slot') is not none }}`, id: 'slot_start' });
        choose.push({ conditions: [{ condition: 'template', value_template: `{{ trigger.id == 'slot_start' }}` }], sequence: [notifyAction(startMsg)] });
      }
      if (trig === 'end' || trig === 'both') {
        triggers.push({ platform: 'template', value_template: `{{ state_attr('${scheduleEntityId}', 'current_slot') is none }}`, id: 'slot_end' });
        choose.push({ conditions: [{ condition: 'template', value_template: `{{ trigger.id == 'slot_end' }}` }], sequence: [notifyAction(endMsg)] });
      }
    ...
    _bindPopupEvents(dlg, ps, magnetPoints) {
      ...
      if (ps.domain === 'climate') {
        ...
        bindChk('.chk-hvac','.hvac-select','hvacMode','enableHvac');
        bindChk('.chk-preset','.preset-input','presetMode','enablePreset');
        bindChk('.chk-fan','.fan-input','fanMode','enableFan');
        bindChk('.chk-swing','.swing-input','swingMode','enableSwing');
      } else if (ps.domain === 'light') {
    ...
    render() {
      ...
      root.querySelectorAll('.chip-menu').forEach(btn=>btn.addEventListener('click',e=>{
        ...
        if(!open){
          ...
          // position:fixed → anchor to the button (escapes hdr-row2 overflow clip)
          const r=btn.getBoundingClientRect();
          const w=dd.offsetWidth||140;
          let left=r.left;
